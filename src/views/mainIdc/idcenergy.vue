    ...
    <div class="highest">
        ...
        <div class="pigeIdc-main">
            ...
            <div class="subject-con none-border-t">
                <p class="subject">IDC赋能发布</p>
            </div>
            <div class="subject-con none-border-t" style="height: 30px;line-height: 30px;width: 1200px;margin: 0 auto;">
                <button class="btn btn-default" id="submitProduct" @click="re1lease">发布赋能</button>
            </div>
            ...
            <div class="focuson" v-show="rightmsg">
                <div class="text-div focusonHover" v-for="(item,i) of listRows" :key="i" @click="listdetails(item)">
                    <h4>{{title}}</h4>
                    <p class="text-wz">{{item.energyProdName}}</p>
                    <p class="details">{{item.energyProdIntroduction}}</p>
                    <div class="PermissionsButton">
                        <div v-show="power==1?false:true">
                            <Button class="btn-zd"
                                @click="setKey(item.energyProdId,item.recommendationFlag,item.ergyProdName)"
                                v-if="item.recommendationFlag==0?true:false">设置重点产品</Button>
                            <Button class="btn-zd" @click="cancelKey(item.energyProdId,item.recommendationFlag)"
                                v-else>取消重点产品</Button>
                        </div>
                        <Button class="btn-sc" @click="deleteKey(item.energyProdId,item.energyProdName,item.creator)"
        ...
        <!-- 尾部 -->
        <div class="subject-cons focusonHover">
            <p class="subjects">重点赋能</p>
            <div class="focusons">
                <div class="text-divs focusonHover" v-for="(item,i) of this.bottomKeyList" :key="item.indexId"
                    v-show="item.infoType=='energy'?true:false">
                    <h4>{{item.infoName}}</h4>
                    <p class="text-wzs">{{item.infoIntroduction}}</p>
                    <p class="detailss" @click="listdetails(item)">了解详情</p>

                    <Button class="btn-zds" @click="cancelKey(item.prodID,item.recommendationFlag)">取消重点产品</Button>
                </div>
    ...
    export default {
        ...
        methods: {
            ...
            search(currentPage) {
                ...
                that.$request({
                    methods: "get",
                    url: "/kapply/idcEnergy/findEnergyProdList",
                    params: {
                    ...
                    onOk: (val) => {
                        that.$request({
                            ...
                            if (res.data.DataRows == '0000') {
                                ...
                                that.popUpTitle = "该目录下有子级目录，不能删除"
                                that.time();
                            } else if (res.data.DataRows == '8802') {
                                that.popUpTitle = "该目录下有产品，不能删除"
                                that.time();
                            } else if (res.data.DataRows == '8803') {
                                that.popUpTitle = "该目录下有草稿，不能删除"
                                that.time();
                            } else if (res.data.DataRows == '8800') {
                                that.popUpTitle = "用户无权限"
                                that.time();
                            }
                            //    console.log(res);
            ...
            setKey(id, suc, name) {
                ...
                for (var i of that.listRows) {
                    ...
                    if (i.energyProdId == id) {
                        i.recommendationFlag = suc;

                    }
                }
                let energyProdID = { "energyProdID": id, }
                that.$request({
                    method: "POST",
                    // url: "/kapply/idc/setImportant?prodID=444",
                    url: "/kapply/idcEnergy/setImportantEnergy",
                    headers: {
                        "Content-Type": "application/json"
                    },
                    // data:prodID
            ...
            cancelKey(id, suc) {
                ...
                that.$request({
                    method: "POST",
                    url: "/kapply/idcEnergy/cancelImportantEnergy",
                    headers: { "content-Type": "application/json" },
                    params: {
                        "energyProdID": id
                    }
            ...
            //跳转赋能详情
            listdetails(item) {
                // console.log(this.bottomKeyList)
                console.log(item);
               
                let proId = item.energyProdId;
                let that = this;
                that.$request({
                    methods: "get",
                    url: "/kapply/idcEnergy/findEnergyProdDetail",
                    params: { "prodID": proId }
                }).then(res => {
                    console.log(res);
                    ...
                    this.$router.push("/energyDetails")
                }).catch(function (error) {
                    // console.log(error)
                })
            },
            ...
            theThree() {
                ...
                that.$request({
                    method: "get",
                    url: "/kapply/idcCode/findEnergyMenu",
                }).then(res => {
                    that.list = {};
                        ...
                        if (i.parentID == 0) {
                            ...
                            for (let j of str) {
                                if (i.codeID == j.parentID) {
                                    ...
                                    //     'content': {}
                                    // }
                                    this.$set(this.list[i.codeID]["content"], j.codeID, {
                                        'title': j.codeName,
                                        "parentID": j.parentID,
                                        "codeID": j.codeID,
                                        'content': {}
                                    ...
                                    for (let x of str) {
                                        if (j.codeID == x.parentID) {
                                            ...
                                            //     'content': {}
                                            // }
                                            this.$set(this.list[i.codeID]["content"][j.codeID]["content"], x.codeID, {
                                                'title': x.codeName,
                                                "parentID": x.parentID,
                                                "codeID": x.codeID,
                                                'content': {}
...
<style scoped>
    ...
    .focusonHover:hover {
        border-radius: 1px solid #00c1de;
